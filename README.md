...
To replicate our results, one can simply run `run.py` with the same parameters as the ones described in the report and with `method=snail`.
...
```bash
python run.py exp.name={exp_name} method=snail dataset=tabula_muris backbone.layer_dim=[64,64] lr=0.001 method.stop_epoch=60
```
